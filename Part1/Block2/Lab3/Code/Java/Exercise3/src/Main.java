...
public class Main {
    final static int minMatrixOrder = 2;
    ...
    public static double findBestSum(double[][] matrix)
    {
        double bestSum = 0;
        for (int i = 0; i < matrix.length; i++)
        {
            ...
            {
                if (j + 1 < matrix[0].length && matrix[i][j] + matrix[i][j + 1] > bestSum)
                    bestSum = matrix[i][j] + matrix[i][j + 1];
                if (i + 1 < matrix[0].length && matrix[i][j] + matrix[i + 1][j] > bestSum)
