...
const int MIN_MATRIX_ORDER = 3;

void initialization(double**& matrix, const int size)
{
...
void free(double**& matrix, const int size)
{
    if (matrix != nullptr)
    {
...
double** inputElementsOfMatrixFromConsole(int& size)
{
    bool isIncorrect = true;
...
double** inputElementsOfMatrixFromFile(std::ifstream& in, int& size)
{
    double** matrix;
    initialization(matrix, size);
    bool isCorrect = true;
...
void input(double**& matrix, int& size)
{
    if (inputMethod())
    {
        ...
        size = inputSizeOfMatrixFromConsole();
        // input elements of matrix
        matrix = inputElementsOfMatrixFromConsole(size);
    }
    else
    {
        bool isNotExit = true;
        ...
        do
        {
            ...
            in.open(fileName);
            // input size of matrix
            size = inputSizeOfMatrixFromFile(in);
            // input elements of matrix
            matrix = inputElementsOfMatrixFromFile(in, size);
            if (size != 0)
            {
                isNotExit = false;
            }
...
double findSum(double**& matrix, const int size)
{
    double sum = 0.0;
    int i = 0;
    int j = 0;
